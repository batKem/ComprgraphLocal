...
bool Texture::createSunBillboardTexture()
{
    ...
    **/
    ...
    int mid_h = height / 2;
    int r = 150;
    float fac = 1.0f/( pow( (mid_w*mid_w) + (mid_h*mid_h),  0.5) - r ); 

    for (int col = 0; col < width; ++col) {
        for (int row = 0; row < height; ++row) {
            img[(row * width + col) * 4 + 0] = 90;//218; // R
            img[(row * width + col) * 4 + 1] = 182;//120; // G
            img[(row * width + col) * 4 + 2] = 255;//0; // B
            //opa
            img[(row * width + col) * 4 + 3] = 175;
            /*
            // dist to center
            float dist = pow ((col-mid_w)*(col-mid_w) +(row-mid_h)*(row-mid_h), 0.5);
            float margin = 55+r;
            if (dist <= r){
                img[(row * width + col) * 4 + 3] = 255; // A
            //} else if (dist <= margin){
             //   img[(row * width + col) * 4 + 3] = 255 *(0.5+pow(1- fac * margin,0.07)); // a
            } else {
               // img[(row * width + col) * 4 + 3] = 255-255* pow(fac * (dist-r),0.07); // a
                //img[(row * width + col) * 4 + 3] = 255-255* pow(fac * (dist-r),0.07); // a
                img[(row * width + col) * 4 + 3] = 255* (exp(-10*fac * (dist-r))); // a
